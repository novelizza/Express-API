...
function load(req, res, next, id) {
    Setting
        ...
        .then((result) => {
            if (!result) {
                setContent(404, errorCodes.settingUSError.SETTING_NOT_FOUND);
...
function update(req, res) {
    ...
    return req.setting.update(req.body)
        .then(() => {
            setContent(200, 'OK');
            return res.status(200).json(getContentSuccess());
        })
...
function list(req, res) {
    const ordering = simpleOrdering(req, 'setting_id');
