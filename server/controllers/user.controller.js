...
function load(req, res, next, id) {
    User
        ...
        .then((result) => {
            if (!result) {
                setContent(404, errorCodes.userUSError.USER_NOT_FOUND);
...
function list(req, res) {
    const ordering = simpleOrdering(req, 'user_id');
    ...
    return User
        ...
        .then((result) => {
            if (!result) {
                setContent(404, errorCodes.userUSError.MAINTENANCE_NOT_FOUND);
                return res.status(404).json(getContentFail(req));
            }
            setContent(200, result);
            return res.status(200).json(getContentSuccess());
